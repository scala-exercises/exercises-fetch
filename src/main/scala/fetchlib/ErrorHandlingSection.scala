...
object ErrorHandlingSection extends FlatSpec with Matchers with Section {
  ...
  def catsEval(res0: Left[FetchException, User]) = {
    ...
    safeResult.value shouldBe res0 //Left(fetch.UnhandledException)
  }
  ...
  def debugDescribe(res0: Left[FetchException, User]) = {
    ...
    value shouldBe res0 // Left(fetch.UnhandledException)
  ...
  def oneRequest(res0: Left[FetchException, User]) = {
    ...
    value shouldBe res0 //Left(fetch.NotFound)
  ...
  def missing(res0: String, res1: String) = {
    ...
    value match {
      case Left(mi @ MissingIdentities(_, _)) => {
        println("Missing identities " + mi.missing) shouldBe res0 // Missing identities Map(User -> List(5, 6))
        println("Environment " + mi.env) shouldBe res1            // Environment FetchEnv(InMemoryCache(Map()),Queue())
      }
      case _ => {}
    }
